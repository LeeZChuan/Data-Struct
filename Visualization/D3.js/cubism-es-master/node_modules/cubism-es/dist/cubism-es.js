(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('d3-dispatch'), require('d3-time-format'), require('d3-fetch'), require('d3-axis'), require('d3-selection'), require('d3'), require('d3-scale'), require('d3-format'), require('d3-interpolate')) :
  typeof define === 'function' && define.amd ? define(['exports', 'd3-dispatch', 'd3-time-format', 'd3-fetch', 'd3-axis', 'd3-selection', 'd3', 'd3-scale', 'd3-format', 'd3-interpolate'], factory) :
  (factory((global.cubism = {}),global.d3Dispatch,global.d3TimeFormat,global.d3Fetch,global.d3Axis,global.d3Selection,global.d3,global.d3Scale,global.d3Format,global.d3Interpolate));
}(this, (function (exports,d3Dispatch,d3TimeFormat,d3Fetch,d3Axis,d3Selection,d3,d3Scale,d3Format,d3Interpolate) { 'use strict';
    ...
    return {
      metric: function metric(request, name) {
        var metricState = {
          ...
          _size: context.size(),
          _values: [],
          _event: d3Dispatch.dispatch('change'),
          _listening: 0,
          _fetching: false
        };
        ...
        return {
          ...
          metric: function metric(expression) {
            return context.metric(function (start, stop, step, callback) {
              d3Fetch.json(host + '/1.0/metric' + '?expression=' + encodeURIComponent(expression) + '&start=' + d3TimeFormat.isoFormat(start) + '&stop=' + d3TimeFormat.isoFormat(stop) + '&step=' + step).then(function (data) {
  ...
  var formatSeconds = d3TimeFormat.timeFormat('%I:%M:%S %p');
  var formatMinutes = d3TimeFormat.timeFormat('%I:%M %p');
  var formatDays = d3TimeFormat.timeFormat('%B %d');
    ...
    return {
      render: function render(selection) {
        ...
        context.on('change.axis-' + id, function () {
          g.call(_axis);
          if (!tick) tick = d3Selection.select(g.node().appendChild(g.selectAll('text').node().cloneNode(true))).style('display', 'none').text(null);
        });
  ...
  var apiOrient = function apiOrient(axisSate) {
    return {
      orient: function orient(_orient) {
        ...
        switch (_orient) {
          case 'top':
            axisSate._axis = d3Axis.axisTop().scale(context._scale);
            break;
          case 'bottom':
            axisSate._axis = d3Axis.axisBottom().scale(context._scale);
            break;
          case 'left':
            axisSate._axis = d3Axis.axisLeft().scale(context._scale);
            break;
          case 'right':
            axisSate._axis = d3Axis.axisRight().scale(context._scale);
            break;
          case 'default':
    ...
    return {
      axis: function axis() {
        var axisState = {
          ...
          _axis: d3Axis.axisBottom().scale(context._scale),
      ...
      render: function render(selection) {
        ...
        selection.each(function (d, i) {
          var id = ++context._id,
              ...
              step = context.step(),
              canvas = d3Selection.select(this).select('canvas'),
              span = d3Selection.select(this).select('.value'),
              m = colors_.length >> 1;
    ...
    return {
      horizon: function horizon() {
        ...
        var state = {
          ...
          _scale: d3Scale.scaleLinear().interpolate(d3Interpolate.interpolateRound),
          _metric: function _metric(d) {
            return d;
          },
          _extent: null,
        ...
        return {
          ...
          metric: function metric(metricInfo) {
            ...
            var metricFn = function metricFn(start, stop, step, callback) {
              ...
              d3Fetch.json(host + uriPathPrefix + 'graph.php?' + constructGangliaWebRequestQueryParams()).then(function (result) {
    ...
    return {
      fire: function fire(isdate, iedate, step, callback_done) {
        ...
        /*
             ...
             */
        d3Fetch.json(full_url).header('X-Requested-With', 'XMLHttpRequest').header('Authorization', auth_string).header('Librato-User-Agent', 'cubism/' + cubism.version).then(function (data) {
      ...
      graphite: function graphite() {
        ...
        return {
          ...
          find: function find(pattern, callback) {
            d3Fetch.json(host + '/metrics/find?format=completer' + '&query=' + encodeURIComponent(pattern)).then(function (result) {
          ...
          metric: function metric(expression) {
            ...
            var metric = context.metric(function (start, stop, step, callback) {
              ...
              d3Fetch.text(host + '/render?format=raw' + '&target=' + encodeURIComponent('alias(' + target + ",'')") + '&from=' + DateFormatter$1(start - 2 * step) + // off-by-two?
              '&until=' + DateFormatter$1(stop - 1000)).then(function (text) {
  ...
  var initState = function initState(context) {
    return {
      ...
      _formatPrimary: d3Format.format('.2s'),
      _formatChange: d3Format.format('+.0%'),
  ...
  var config = {
    ...
    serverDelay: 5e3,
    clientDelay: 5e3,
    event: d3Dispatch.dispatch('prepare', 'beforechange', 'change', 'focus'),
    start0: null,
    ...
    timeout: null,
    focus: null,
    scale: d3Scale.scaleTime().range([0, 1440])
  };
  ...
  var context$1 = function context() {
    var state = {
      ...
      _serverDelay: 5e3,
      _clientDelay: 5e3,
      _event: d3Dispatch.dispatch('prepare', 'beforechange', 'change', 'focus'),
      _start0: null,
      ...
      _timeout: null,
      _focus: null,
      _scale: d3Scale.scaleTime().range([0, 1440])
    };
    ...
    d3Selection.select(window).on('keydown.context-' + ++_context._id, function () {
      switch (!d3Selection.event.metaKey && d3Selection.event.keyCode) {
        case 37:
          // left
...
//# sourceMappingURL=cubism-es.js.map
