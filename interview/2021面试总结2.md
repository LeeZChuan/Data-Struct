...
一、静态页面对网站SEO优化的好处
       ...
       1、更容易抓取
       因为静态页面的一次性抓取，让百度蜘蛛可以一次性的抓取页面，而动态页面的代码需要多次调取而影响了蜘蛛的抓取，因此静态页面提高了页面的抓取率和收录率。

       2、打开速度更快
       我们都知道网站的打开速度是影响网站排名的一个重要因素，由于静态页面的数据调取是一次性的从服务器调取，避免了动态页面的多次调取，所以相对来说静态页面的加载速度更快。

       3、更安全
       使用静态页面，网站的安全系数也更高，因为静态页面网站只是从服务器调取一次数据，从而减少了被攻击的可能，只要做好后台的隐藏，保护好网站的登录密码，即可降低网站被黑的几率。

对于seo优化要求来说，伪静态和纯静态都能达到搜索引擎的要求，至于什么时候用伪静态，什么时候用纯静态，一般我们的内容不经常变的，比如新闻、资讯，如果可以尽量用纯静态，而对于要时时刻刻动态请求的，比如多分类页面筛选、tag标签、搜索等需要在页面渲染时执行一定的逻辑的就可以用伪静态了。
...
> http header上有一个字段叫accept-encoding:gzip,设置成gzip就可以了
