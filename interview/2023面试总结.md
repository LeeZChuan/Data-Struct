...
https://github.com/chenshenhai/koajs-design-note/blob/master/note/chapter01/03.md
...
> 将资产单据发起抽象出来，由一个入口进入进行分发，这样代码修改成本会减少，并且便于扩展
...
```js
// 超时时间 timeout: 120000,
// 在fail方法中回调,如果是网络连接失败

// code: "ERR_NETWORK"
// config: {transitional: {…}, adapter: Array(2), transformRequest: Array(1), transformResponse: Array(1), timeout: 0, …}
// message: "Network Error"
// name: "AxiosError"
// request: XMLHttpRequest {callbacks: {…}, onloadstart: null, onload: null, onabort: ƒ, onerror: ƒ, …}
// stack: "AxiosError: Network Error↵    at XMLHttpRequest.handleError [as onerror] (http://127.0.0.1:
...
在JavaScript中，判断对象是否存在循环依赖是一个比较复杂的问题。一种常见的解决方法是使用弱引用来判断对象是否被引用过。你可以使用WeakSet来存储已经遍历过的对象，然后在遍历对象属性时，判断属性值是否存在于WeakSet中。如果存在，则说明存在循环依赖。
...
console.log(hasCircularDependency(obj1)); // 输出 true
```
请注意，该方法只能判断直接循环依赖，无法判断间接循环依赖。在实际项目中，避免出现对象循环依赖是一个好的编程实践。
...
```js
// 我们的项目核心就是formdata， params.file
// let formData = new FormData();
//   formData.append('pic', params.file, params.fileName || params.file.name);
```
...
不常用的方式：使用table布局：使用 display: table 和 display: table-cell 居中
...
> LocalStorage译为“本地存储器”,SessionStorage译为“会话存储”
...
2、根据同源原则，不同浏览器不能共享loacalStorage和sessionSrorage的信息，相同的浏览器不同页面可以共享loacalStorage（必须相同域名和端口），但是相同浏览器            
不同页面或者标签不能共享sessionStorage信息（sessionStorage仅仅存储在当前页面），这个页面是指顶级窗口页面，如果一个页面有几个不同的iframe标签，是可以共享sessionStorage信息。

用法：两者用法差不多，但是取值有所不同

> LocalStorage生命周期是永久,除非主动清除LocalStorage信息，否则这些信息将一直存放在客户端。而SessionStorage生命周期是临时，仅在当前会话窗口有效，关闭页面或浏览器数据就会自动被清除。

> 所以比如需要储存一些长期下次还需要访问的数据，用户信息使用LocalStorage，对于只是暂时储存，后端穿入的数据使用SessionStorage

#### 10. 说说防抖截流是什么怎么用的？

> 防抖 (Debouncing) 的含义是指在一定时间内，多次触发同一个事件，只执行最后一次操作。

使用场景：当我们在搜索框中输入关键词时，输入框会不断触发 oninput 事件，如果每次输入都去请求服务器获取数据，会造成不必要的请求浪费。

> 节流 (Throttling) 的含义是指在一定时间内，多次触发同一个事件，只执行第一次操作。

使用场景：例如，当我们拖动网页上的滚动条时，会不断触发 onscroll 事件，如果每次触发都去计算滚动距离，会造成浏览器性能下降。

#### 11. 什么是闭包？

> 闭包就是定义一个方法a，在这个方法内返回一个内部定义的方法，让这个这个方法外部访问他，形成的这个流程就是闭包
...
#### 2. 说说vue2与vue3的响应式有什么区别？为什么Object.defineProperty不能监听数组

> https://blog.csdn.net/Android_boom/article/details/127318875
...
const obj = new Proxy(data, {
    // getter 拦截读取操作
    get(target, key) {
        // 将副作用函数 activeEffect 添加到存储副作用函数的全局变量 targetMap 中track(target, key)// 返回读取的属性值return Reflect.get(target, key)},// setter 拦截设置操作set(target, key, val) {// 设置属性值const result = Reflect.set(target, key, val)// 把之前存储的副作用函数取出来并执行trigger(target, key)return result}
})
// 存储副作用函数的全局变量
const targetMap = new WeakMap()
// 在 getter 拦截器内追踪依赖的变化
function track(target, key) {// 没有 activeEffect，直接返回if(!activeEffect) return// 根据 target 从全局变量 targetMap 中获取 depsMaplet depsMap = targetMap.get(target)if(!depsMap) { // 如果 depsMap 不存，那么需要新建一个 Map 并且与 target 关联 depsMap = new Map() targetMap.set(target, depsMap)}// 再根据 key 从 depsMap 中取得 deps, deps 里面存储的是所有与当前 key 相关联的副作用函数let deps = depsMap.get(key)if(!deps) { // 如果 deps 不存在，那么需要新建一个 Set 并且与 key 关联 deps = new Set() depsMap.set(key, deps)}// 将当前的活动的副作用函数保存起来deps.add(activeEffect)
}
// 在 setter 拦截器中触发相关依赖
...
#### 5. 说说cssloader与styleloader有什么用

> 都是因为webpack是对js的打包插件

经过css-loader的转译，我们已经得到了完整的css样式代码，style-loader的作用就是将结果以style标签的方式插入DOM树中。 但css-loader返回的不是css样式代码的文本，而是一个js模块的代码，将这些js代码直接放进style标里显然是不行的。

#### 6. 口述一下promise怎么实现的吧？

Promise 是通过构造函数的方式实现的，它接受一个函数作为参数，该函数具有 resolve 和 reject 两个参数，这两个参数可以在 Promise 对象的状态发生变化时被调用。

在 Promise 对象中，有三种状态：pending（进行中）、fulfilled（已完成）和 rejected（已拒绝）。Promise 对象在创建时处于 pending 状态，当 resolve 函数被调用时，它会转变为 fulfilled 状态，而如果 reject 函数被调用，则会转变为 rejected 状态。

Promise 对象可以通过 then() 方法来添加异步操作完成时的回调函数，同时也可以通过 catch() 方法来处理异步操作被拒绝时的情况。

严格来说 Promise 并不是异步操作，它只是用于处理异步操作的结果。实际上，Promise 对象内部的异步操作是由开发人员自己定义的。它可以是一个 Ajax 请求、一个数据库查询或者任何其它能够返回 Promise 对象的异步操作。
#### 7. 刚刚提到了轮询，promise底层是用的轮询吗？

#### 8. 说说你了解的性能优化有哪些？

链接：https://zhuanlan.zhihu.com/p/121056616
...
#### 10. 说说有一个uv几十万的项目之前是使用vue2项目开发的，现在要使用vue3项目进行重构有哪些方法可以实现
#### 10. 如何用最少的时间将vue2的代码转化成vue3的代码
...
#### 12. 现在要你实现一个0-100的进度条，怎么实现

1. 使用JavaScript监听页面加载事件
```js
// 获取进度条元素
...
上述代码中，首先定义了一个高度为5px的进度条元素，并设置了其位置为固定，并使用CSS动画实现进度条效果。在CSS动画中，使用@keyframes关键字定义了一个名为progress-bar的动画，从宽度为0开始，到宽度为100%结束，持续时间为2秒，并使用forwards关键字使动画结束后保持状态。

#### 13. 如果不用css样式实现使用js如何实现呢，记得使用requestAnimationFrame

使用requestAnimationFrame() 这个方法不断循环判断当前的百分比状态

#### 14. 请你实现一个3x3黑白相间的格子如何使用一个div实现


```js
// 尝试对一个空数组完成 0～10000 数字的插入，然后将这个数组按序渲染到页面上，尽可能的以高效、不阻塞UI 的前提下完成此功能。
const logArr=(arr)=>{
  console.log(arr);
}
...
const deepClone = (obj) => {
  ...
  if(obj){
    for (let key in obj) {
      if (obj.hasOwnProperty(obj[key]) && obj !== obj[key]) {
...
const aurl=url(eppandina,b,.com)
const burl=url(baidu,b,.com)
aurl(eedjia)

url('eppandina',b,.'com')(b)

##### 登陆逻辑
// 实现登陆，客户的唯一标识传给后端，token存到cookie，
// 每跳转到一个新页面，判断是否token是否存在，vue router里路由守卫跳转到登录页面，
// 登陆标识的存储，在什么判断，也就是在beforeRouter

// localstorage，长久存储，员工端管理端，身份标识比较固定可以储存
// seesionstorage，短期储存，这一次能用的，比如useinfo用户信息，独有性，只有这个用户才有的，每次登陆因为每次登陆变动而变动
// cookie是存放token


##### v3路由跳转
```js
// TODO: 加载文件的 loading , 或者页面进入失败, 可以提示.
router.beforeEach(async (gurd, from) => {
  // 是否是管理员身份
  const normalIdentify = localStorage.getItem('normalIdentify') == 'true'
  dispatchIsAdmin(!normalIdentify)

  if ((currentUser.value && currentUser.value.id) || isUnAuthRoute(gurd)) { // selfToken钉钉员工登录标识
    return Promise.resolve()
  }
  
  if (isAdmin.value) {
    return setAdminBaseInfo()
  } else {
    return setEmployeeBaseInfo()
  }
})
```
...
```js
    ...
    if(res.code==="ERR_NETWORK"){
        // 错误数据上报这样的问题--存储到loaclstorage
        return responseHandler({
          code: 999,
          data: null,
          message: '网络繁忙中断，请稍后重试!',
          requestId: '没有抓住requestId~',
        });
      }
...
// 或者考虑使用市面上的开源框架：Sentry 是一个实时的事件日志和聚合平台
...
> 1. 架构更新：使用抽象和面向对象的逻辑去实现目前的业务逻辑--都是为了解决开发效率

> 2. 技术更新：使用一套代码对多端，之前是多套代码开发周期长--都是为了解决开发效率

> 3. 用户体验，更新用户体验相关的问题，交互和用户ui需要迭代更新，比如单据发起，


> 项目亮点：使用i18n进行国际化

> 1. export const t = Vuei8n.global.t


#### vue2如何迁移到vue3，要做哪些事情
...
#### 1. 说说ts到底做了什么，有哪些优势和劣势


#### 2. 给你一个论坛的项目，你说说从前端到后端设计应该如何实现
...
#### 一面：
1.假设按照升序排序的数组在预先未知的某个点上进行了旋转。(例如，数组[0,1,2,4,5,6,7]可能变为[4,5,6,7,0,1,2])。搜索一个给定的目标值，如果数组中存在这个目标值，则返回它的索引，否则返回-1。你可以假设数组中不存在重复的元素。你的算法时间复杂度必须是O(logn)级别。
2.asyncfunctionasync1(){console.log('async1start');awaitasync2();console.log('async1end');}asyncfunctionasync2(){console.log('async2');}console.log('scriptstart');setTimeout(function(){console.log('setTimeout');},0)async1();newPromise(function(resolve){console.log('promise1');resolve();}).then(function(){console.log('promise1then');return'promise1end';}).then((res)=>{console.log(res);}).then((res)=>{console.log(res);});console.log('scriptend');
3.BFC
4.浏览器渲染流程
5.XSS CSRF
6.手写节流
...
1.自我介绍；
2.项目；
3./*现有一个含有字符串的数组，形如：["ab","c","ab","d","c"]要求将其中出现的重复字符串，依次添加上数字序号，如：["ab1","c1","ab2","d","c2"]要求：1).未重复出现的字符串不处理；2).仅对相同的一组字符串依次添加序号，而不是共用一组序号；3).保持原数组顺序；*/functionaddSerial(arr){varresult=[]//dosomethingreturnresult}console.log(addSerial(["ab","c","ab","d","c"]));
4.inner='window'functionsay(){console.log(inner)console.log(this.inner)}varobj1=(function(){varinner='1-1'return{inner:'1-2',say:function(){console.log(inner)console.log(this.inner)}}})()say()obj1.say()obj1.say=sayobj1.say()；
5.你的优缺点；
6.你有什么想问我的：没有。

先介绍项目，然后问了一道基础题，浏览器从输入url到页面加载完成这整个流程是怎样的，然后问了几道算法题：
1）二叉树分层遍历，输出层数和该层节点值；
2）把一块区域矩阵化为2维矩阵，m行n列，矩阵中元素只有0或1两个值，0表示水，1表示陆地，相邻的陆地可以连接成一块更大的陆地，问这mxn的矩阵中共有几块陆地3)有一份系统的日志文件，每行记录都有uid(用户id),login_time(登入时间),logout_time(登出时间)三个字段，问系统在线峰值人数，及峰值持续的最长时间不知道是不是算法题都这种难度。

prototype和__proto__for in和for ofaxios方法，怎么用，流程cookie用法，参数js去重有哪些方法 不记得了。。
...
function Dog(name, color) {    
    ...
    this.bark = function() {        
        ...
        Dog.prototype.bark = function () {    console.log(`hi ${this.name}`)}let dog = new Dog('Chop', 'Red');dog.bark();dog.__proto__.bark.call(dog);
...
> js是单线程的：JavaScript代码在执行的任何时候，都只有一个主线程来处理所有的任务。
...
#### 2. 50个接口进行请求，每3个接口进行请求，当返回后继续push进去新的接口然后继续请求，请问如何设计

#### 3. 接口数量不定，请不要用promise.all方法设计一个方法，让其一直存在loading加载动画


#### 4. 请实现一个虚拟列表，如何实现，请说说原理

使用paddingTop与paddingBottom的属性进行动态化偏移
...
#### 6. 说说你了解的pinia与vuex有啥区别，并且有啥优势


### 美团优选一面

#### 1.说说taro和uniapp的技术选型

Taro技术底层是使用的react，taro只能使用vscode进行开发

uniapp技术底层是基于vue进行的封装，并且uniapp提供了自己的构建器
...
#### 2.如果让你选择框架，你会选择什么框架，你是怎么看待这些框架的
...
微任务队列与宏任务队列同时出现的时候，先执行微任务再执行宏任务
#### 4.说说position这个属性配置把？有哪些

position有relative、abslote、static
...
class Solution {
    public ListNode reverseBetween(ListNode head, int left, int right) {
        ...
        pre.next = null;
        rightNode.next = null;
        revertInner(leftNode);
        pre.next = rightNode;
        leftNode.next = curr;
        return dummyNode.next;

    }
    private void revertInner(ListNode head){
        if(head == null) {
            return ;
        }
...
#### 1. 给你两个数组如何让这两个数组合并并去重？

#### 2. 给你一个数组如何在add这个字符串的前面添加或者修改对应的数组方法？
...
#### 4. 你提到setup，你说说项目中使用setup()如何做页面的优化或者性能优化呢？

#### 5. 为什么说Vue是单页面应用呢？那vue如何实现多页面应用？

> SPA单页面应用：指的是只有一个主页面的应用，一开始只需要加载一次css与js。所有内容都包含在主页面，对每一个功能模块组件化。单页面应用跳转，就是切换相关组件，仅仅只是刷新局部资源。

> MPA多页面应用：指的是有多个独立页面的应用，每个页面都必须重复加载js、css等相关资源。多页面应用跳转，需要整页资源刷新。


区别：

> 1. 刷新方式：SPA，相关组件的切换，页面局部刷新或者更改，MPA需要整页刷新

> 2. 路由模式：SPA：可以使用hash，也可以使用history，MPA：普通的链接跳转

> 3. 用户体验，SPA：一次性加载所有资源，所以首次进入的时候会有一段时间的白屏，MPA:页面切换加载缓慢，流畅度不够

> 4. 转场动画，SPA: 容易实现转场动画，MPA:无法实现转场动画

> 5. 数据传递问题：SPA:容易传递数据，方法有很多，比如Vuex，MPA：传递数据不方便，一般用url、cookie、本地储存

> 6. 搜索引擎优化SEO: SPA: 需要单独方案，实现比较困难，MPA:实现方法容易

> 7. 使用范围：SPA: 高要求的体验度，追求页面流畅。MPA:使用于追求高度支持搜索引擎的应用

> 8. 结构：SPA:一个主页面+许多模块组件，MPA:许多完整的页面

<!-- 如何在vue中实现多页面应用 -->
> 1. iframe
> 2. 微前端
> 3. 在vue项目中使用new App创建多个元素
...
#### 2. 如何实现中文文字首行缩进，英文文字不首航缩进

在CSS中，使用text-indent设置文本的缩进，例如，缩进两个中文的位置，可以设置text-indent为2em。

在CSS中，通过text-transform来设置字符转换，字符的转换主要用于英文，可以通过CSS设置英文的大小写，首字母大写。有四种值可以选择，分别是：
...
```
块元素：块元素在页面中以区域块的形式出现，其特点是，每个块元素通常都会独自占据一行或多行，可以对其设置宽度、高度、对齐等属性，常用于网页布局和网页结构的搭建。

常见的块元素有：

<h1>~<h6>、<p>、<div>、<ul>、<ol>、<li>等，其中<div>标签是最典型的块元素。

行内元素：行内元素也称内联元素或内嵌元素，其特点是，不会占据一行，也不强迫其他的标签在新的一行显示。一个行内标签通常会和其他行内标签显示在同一行中，它们不占有独立的区域，仅仅靠自身的文本内容大小和图像尺寸来支撑结构，一般不可以设置宽度、高度、对齐等属性，常用于控制页面中文本的样式。

常见的行内元素有：
<strong>、<b>、<em>、<i>、<del>、<s>、<ins>、<u>、<a>、<span>等，其中<span>标签最典型的行内元素。
```
...
块级作用域：就是在bfc中的元素内容不会被外层元素样式所影响
...
```js
// 1: 编写一个函数，接受一个字符串，返回一个对象，其中包括每个字符在字符串中出现的次数。

// 1. aabbcc
const string = 'aabbvvdd'
const stringToSum = (string) => {
...
// 问题3:const numberStr='1234567'
// 3: 编写一个函数，接受一个整数n，返回一个包含从1到n的所有数字的字符串，但是在所有3的倍数的地方替换为"Fizz"，在所有5的倍数的地方替换为"Buzz"，如果数字既是3的倍数又是5的倍数则替换为"FizzBuzz"。
...
#### 1. 有一个场景IO操作，请你设计一个让用户可以试试反馈获取这100个文件的读取情况到哪里了？
...
Cors：主要是后端限制header xml http request是否能发送跨域请求，被访问页面的3个respond header，第一个就是域名，cookie和header
...
demi：用来解决编写的代码在vue2和vue3之间的兼容问题
...
1. 用于监控项目的页面性能问题
window.perform.timing

2. 用于监控收集项目的错误信息可以考虑： sentry
<!-- sentry日志监控 -->

#### 如何做到获取当前页面的元素出现的次数(9.统计任意页面中使用率最高的 HTML 标签)

```js
const getHtmlNumber = (): string => {
    // 获取页面中所有的标签HTML数组	
    let tags = document.querySelectorAll('*')
    // 将标签类型和对应的数量存入一个对象中
    let tempArr = Array.from(tags).map(node => node.nodeName)
    // 声明一个对象进行标签储存
    let resultObj = {};
    // 得到最后的标签名称
    let resultHtml = '';
    ...
    tempArr.forEach(item => {
        if (resultObj[item]) {
            ...
            // 这里做一个比较操作
            if (resultNum < resultObj[item]) {
                resultNum = resultObj[item];
                resultHtml = item;
            }
            if (resultNum == resultObj[item]) {
                resultNum = resultObj[item];
    ...
    //     if (resultNum < resultObj[key]) {
    //         resultNum = resultObj[key];
    //         resultHtml = key;
    //     }
    //     if (resultNum == resultObj[key]) {
    //         resultNum = resultObj[key];
    //         resultHtml = `${resultHtml},${key}`;
    //     }
    // }
...
const myJsonStringify=(obj)=>{
    ...
    for(let key in obj){
        ...
        if(!typeof obj[key] == 'object'){
            if(resultString){
                resultString=`${resultString},${key}:${obj[key]}`;
            }else{
                resultString=`{${key}:${obj[key]}`;
            }
        }else{
            resultString=`${resultString},${myJsonStringify(obj[key])}`;
        }
    }
    return `${resultString}}`;
}
...
不管是taro还是uni-app，setData的优化都是小程序性能优化中最为重要之事，且优化主要有两个方向：
...
前面调研时知道megalo是做了差量数据更新的，那就应该问题不大。
...
keep-alive：vue2.0提供了一个keep-alive组件用来缓存组件,避免多次加载相应的组件,减少性能消耗，所以确保vue 版本为2.0 以上
...
#### 3.
定义和用法 <hr> 标签在 HTML 页面中创建一条水平线。 水平分隔线（horizontal
rule）可以在视觉上将文档分隔成各个部分。   定义和用法 <br> 可插入一个简单的换行符。 <br>
标签是空标签（意味着它没有结束标签，因此这是错误的：<br></br>）。在 XHTML
中，把结束标签放在开始标签中，也就是 <br />。 请注意，<br> 标签只是简单地开始新的一行，而当浏览器遇到
<p> 标签时，通常会在相邻的段落之间插入一些垂直的间距。   定义和用法 <tr> 标签定义 HTML 表格中的行。
...
#### 5.
 <br> 可插入一个简单的换行符。 <br> 标签是空标签（意味着它没有结束标签，因此这是错误的：<br></br>）。在 XHTML 中，把结束标签放在开始标签中，也就是 <br />。 请注意，<br> 标签只是简单地开始新的一行，而当浏览器遇到<p>标签时，通常会在相邻的段落之间插入一些垂直的间距。
