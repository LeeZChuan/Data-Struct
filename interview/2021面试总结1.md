...
- [浩鲸科技（外包）](#浩鲸科技外包)
  ...
  - [vue或者react组件间通信可以使用那些方法](#vue或者react组件间通信可以使用那些方法)
  - [原型原型链是什么](#原型原型链是什么)
  - [ES6有哪些新特性](#es6有哪些新特性)
...
* 使用promise、async、await的异步操作都会加入到微任务队列中，他会在调用栈执行为空的情况的情况下立即执行，
...
* 基本数据类型：string、number、undifind、bool、symbol、bigint
...
> 缓存区是 Git 中的一个中间区域，也被称为暂存区。它是一个文件，用于暂存开发者在工作区中所做的修改，这些修改还没有提交到仓库中。开发者在将修改提交到仓库中之前，可以使用 Git 的其他命令在缓存区中进行修改和调整。

> 工作区是指开发者在本地计算机上的工作目录，里面包含了代码、文档、图片等所有项目文件。当开发者在工作区修改了代码后，这些修改并没有立即提交到 Git 仓库中，而是需要通过 Git 的其他命令将这些修改提交到暂存区或直接提交到仓库中。

> git add：将工作区中的修改添加到缓存区中。例如，要将修改的所有文件添加到缓存区中，可以使用命令“git add .”，其中“.”表示当前目录下的所有文件。
...
* 原型：分为显示原型与隐式原型
* 显示原型：Object.prototype，隐式原型：Object._proto_,引用函数的原型是指向构造函数的原型链
...
### typescript的interface与class有什么区别

一个是接口另一个是类

class可以被继承但是，interface不可被继承

### 简单介绍一下你对canvas与svg的理解

### 如果区分判断不同的浏览器或者不同内核

使用媒体查询

### 媒体查询你了解吗（用于解决判断不同浏览器不同内核的情况）

* 什么是媒体查询，如何理解？:就是根据不同的屏幕或设备，自适应布局。一套代码，多个设备应用，合理布局。
* 在网页的头部添加这行代码
```html
<meta content="width=device-width,initial-scale=1.0,maximum-scale=1.0,user-scalable=no" name="viewport">
```

　　其中：width=device-width: 网页宽度等于设备宽度
...
### typescript如何实现多种赋值，比如声明一个a对他进行赋值string与number两种类型
...
* svg与canvas的事件绑定不一样，
* svg因为具有一定的解构，树与保留模式的渲染，或者说，是有js方法可以直接操作的
* canvas里所有的对象其实都是假的，需要自己处理事件
* svg有自己的类dom操作的api
* canvas只是一个渲染视图


> 需要响应一些，就svg，如果性能一些就canvas
> 内容多复杂动效，3d，选canvas。元素少，svg
> 
...
### 为什么说settimeout的time是最小时间

### 聊聊你的可视化学习历程是什么样的


### 算法题，如何实现一个小球从高空下落，每次返回上次高度的一半
